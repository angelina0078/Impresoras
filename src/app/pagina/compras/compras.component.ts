...
export class ComprasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formularioCompra = this.fb.group({
      ...
      codigoPostral: ['', Validators.required],
  ...
  generarPDFModal():void{
    ...
    this.factura.productos.forEach((item:any, index:number) => {
      y +=10;
      doc.text(
        `${index+1}.${item.productos.nombre}-cantidad: ${item.cantidad}- precio:$${item.productos.precio.toFixed(2)}- subtotal: $${(item.productos.precio*item.cantidad.toFixed(2))}`,
        20,
        y
      )
